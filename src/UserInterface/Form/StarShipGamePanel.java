...
public class StarShipGamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    double shipVelocityX = tileSize*1.5; //ship moving speed
    Block ship;
    ...
    int bulletVelocityY = -12; //bullet moving speed
    ...
    public StarShipGamePanel() {
        ...
        //move timer
        moveTimer = new Timer(100, new ActionListener() { // 100 ms delay
            @Override
            public void actionPerformed(ActionEvent e) {
                if (leftPressed && ship.x - shipVelocityX >= 0) {
                    ship.x -= shipVelocityX;
                }
                if (rightPressed && ship.x + shipVelocityX + ship.width <= boardWidth) {
                    ship.x += shipVelocityX;
                }
            }
        });
        ...
        for (int i = 0; i < alienColumns * alienRows; i++) {
            ...
            do {
                ...
                for (Block alien : alienArray) {
                    if (detectCollision(new Block(randomX, randomY, alienWidth, alienHeight, null), alien)) {
                        positionValid = false;
                        break;
                    }
                }
